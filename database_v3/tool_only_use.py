...
def make_completion(tool_call: str, tool_result: str, gold: str) -> str:
    return (
        f"<python>\n"
        f"{tool_call.strip()}\n"
        f"</python>\n"
        f"<output>{tool_result}</output>\n"
        f"Based on the Python output, the final answer is \\boxed{{{gold}}}."
    )


def make_record(kind: str, question: str, gold: str, tool_call: str, seed: int | None = None) -> dict:
    tool_result = f"{gold}\n"
    ...
    completion = make_completion(tool_call, tool_result, gold)
