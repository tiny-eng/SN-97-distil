...
def make_pass1_prompt(question: str) -> str:
    return (
        f"{question}\n\n"
...
def make_pass1_assistant(tool_call: str) -> str:
    return (
        f"<python>\n"
        f"{tool_call.strip()}\n"
        f"</python>"
...
def make_pass2_prompt(pass1_prompt: str, text1: str, tool_result: str) -> str:
    return (
        f"{pass1_prompt}\n"
        f"{text1}\n"
...
def make_pass2_assistant(gold: str) -> str:
    return f"\\boxed{{{gold}}}"
...
def make_record(
    ...
    pass1_prompt = make_pass1_prompt(question)
    pass1_assistant = make_pass1_assistant(tool_call)

    pass2_prompt = make_pass2_prompt(
        pass1_prompt=pass1_prompt,
        text1=pass1_assistant,
        tool_result=tool_result,
    )
    pass2_assistant = make_pass2_assistant(gold)
    ...
    if seed is not None:
        base_record["seed"] = seed

    pass1_record = {
        **base_record,
        "pass_id": 1,
        "status": "generated_with_tool_pass1",
        "messages": [
            {"role": "user", "content": pass1_prompt},
            {"role": "assistant", "content": pass1_assistant},
        ],
    }
    ...
    pass2_record = {
        **base_record,
        "pass_id": 2,
        "status": "generated_with_tool_pass2",
        "messages": [
            {"role": "user", "content": pass2_prompt},
            {"role": "assistant", "content": pass2_assistant},
        ],
    }

    return [pass1_record, pass2_record]


def generate_item(kind: str, r: random.Random, seed: int | None = None) -> list[dict]:
    if kind == "shopping_budget":
        ...
        gold_n = start_money - (n_items_a * price_a + n_items_b * price_b)
        ...
        gold_n = n_classes * (students_per_class - absent_per_class) + volunteer_helpers
        ...
        gold_n = n_walls * bricks_per_wall + extra_safety - broken_bricks
        ...
        question = (
            ...
            f"centigrams. Give the integer equal to v1*p1 + v2*p2."
        )
        ...
        digit_sum_value = sum(int(d) for d in str(n_val))
        sign = r.choice(["minus", "plus"])
        offset = r.randint(1, 19)

        if sign == "minus":
            gold_n = digit_sum_value * target - offset
            op = "-"
        else:
            gold_n = digit_sum_value * target + offset
            op = "+"
        ...
        question = (
            ...
            f"it end? Give the answer as the integer HHMM, for example "
            f"03:25 becomes 325 and 11:45 becomes 1145."
        )
...
def build_records(seed: int, n_per_kind: int, shuffle: bool = True) -> list[dict]:
    ...
    for kind in KINDS:
        for _ in range(n_per_kind):
            ...
            item_records = generate_item(kind, item_rng, seed=item_seed)
...
def main():
    parser = argparse.ArgumentParser(
        description="Build a variable-rich JSONL Python tool-use math database."
    )
    ...
    for record in records:
        kind = record["kind"]
        pass_id = record.get("pass_id", "unknown")

        counts[kind] = counts.get(kind, 0) + 1
        pass_counts[pass_id] = pass_counts.get(pass_id, 0) + 1

    total_problems = len(KINDS) * args.n_per_kind
    ...
    print(f"Problems per kind: {args.n_per_kind}")
    print("Rows per problem: 2")
    print(f"Append mode: {args.append}")

    print("\nPass counts:")
    for pass_id in sorted(pass_counts):
        print(f"  pass {pass_id}: {pass_counts[pass_id]}")
