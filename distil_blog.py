...
class DataStream:
    ...
    def __init__(self, min_chars=2560):
        ...
        self._ds = iter(load_dataset(DATASET, split="train", data_files="shard_00000.parquet", streaming=True))
...
def main():
    ...
    parser.add_argument("--teacher_gpu", type=str, default=0)
    ...
    log.info(f"Loading teaher ({args.teacher}) on GPU {args.teacher_gpu}...")
    ...
    while True:
        ...
        tokens = [
            tokenizer(t, return_tensors="pt", truncation=True, max_length=args.max_seq_len).input_ids.squeeze(0) for t in texts
        ]
        ...
        log.info(f"Step {global_step} | KL: {avg_kl: .4f} | LR: {lr: .2e} | " f"{elapsed:.1f}s ({n/elapsed:.1f} samp/s) | pos: {data.position:,}")

        if not args.no_wandb:
            import wandb
            wandb.log({"train/kl": avg_kl, "train/lr": lr, "perf/step_time": elapsed}, step=global_step)
